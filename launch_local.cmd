...
start "BoardRAG API" cmd /k pushd "%ROOT%" ^&^& call venv\Scripts\activate.bat ^&^& python -m uvicorn api.main:app --host 127.0.0.1 --port 8000 --reload

REM Start Next.js frontend in a new terminal
if exist "web" (
  start "BoardRAG Web" cmd /k pushd "%ROOT%web" ^&^& npm install ^&^& npm run dev
) else (
